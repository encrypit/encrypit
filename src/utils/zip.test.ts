...
describe('createZipFile', () => {
  ...
  it('creates a zip file', async () => {
    ...
    expect(zip.BlobWriter).toHaveBeenCalledWith(MIME.ZIP);
    expect(zip.ZipWriter).toHaveBeenCalledTimes(2);
    files.forEach((file) => {
      expect(zip.BlobReader).toHaveBeenCalledWith(file);
      expect(mockZipWriter.add).toHaveBeenCalledWith(
        file.name,
        expect.anything(),
      );
    });
    expect(zip.ZipWriter).toHaveBeenCalledTimes(2);
    expect(mockZipWriter.close).toHaveBeenCalledTimes(2);
  });
});
...
describe('unzip', () => {
  ...
  it('unzips a zip file', async () => {
    ...
    expect(zip.BlobWriter).toHaveBeenCalledTimes(1);
    expect(zip.ZipReader).toHaveBeenCalledTimes(1);
    expect(zip.ZipReader).toHaveBeenCalledWith(expect.anything(), {
      password:
    ...
    expect(mockZipReader.close).toHaveBeenCalledTimes(1);
  });
});
